...
csv <- read.table(csv_file, header=TRUE, sep=",", fill=TRUE, stringsAsFactors=FALSE, encoding="UTF-8")

# Data types and scaling
dfFmrData <- data.frame(csv, fmr2Num=NA, pop2010Num=NA)
dfFmrData$fmr2Num <- as.numeric(csv$fmr2)
dfFmrData$pop2010Num <- as.numeric(csv$pop2010) / 1000000

```

```{r, echo=FALSE}
g5 <- ggplot(data=dfFmrData, aes(x=pop2010Num, y=fmr2Num))
g5 <- g5 + geom_point()
g5 <- g5 + theme(axis.text.x = element_text(angle=30, vjust=1))
g5 <- g5 + labs(title="Fair Market Rent for 2 Bedroom Unit vs. 2010 Census",
