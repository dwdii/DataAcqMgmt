...
#### 1.d. ####
# Update the queue to reflect the fact that Patricia has talked her way 
# in front of Steve with just one item.
(queue <- queue[c(1, 4, 2, 3, 5)])
# [1] "Mary"     "Patricia" "Steve"    "Alex"     "Harold" 

#### 1.e. ####
# Update the queue to reflect that fact that Harold has grown impatient
# and left.
(queue <- queue[1:length(queue) - 1])
# [1] "Mary"     "Patricia" "Steve"    "Alex" 
...
#### 2. ####
# Modify your answer to quiz exercise 21 so that when you implement the 
...
#### 4. ####
# Write R code that takes three input constants f, g, and h and determinues 
