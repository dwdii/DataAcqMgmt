...
g1 <- g1 + annotate("Text", label=sprintf("y=%.2f + %fX", fit$coefficients[1], fit$coefficients[2]),
                    y=max(birthsPerYearMth$Births), 
                    x=lubridate::parse_date_time("2010-12-01", "ymd"),
                    size=3)
g1
```

Definitely a negative trend overall, but not a drastic one... the slope is only `r sprintf("%f", fit$coefficients[2])`.  And notice that when we differentiate pre-2011 versus 2010 onward, the sign of the slope flips. After 2010, there is a definite rise in births. Ok, lets bring unemployment data in the picture during this same time frame.
...
![Visualization of U.S. Births December 2009](C:\\Code\\R\\DataAcqMgmt\\FinalProject\\Charts\\2009_12_US_Births.png)
