...
Using the functions defined above, let's bring the natality, census and shape data into our session.
...
We can determine how many births occurred each month by aggregating the birth data by date. The `Date` column in this case is at a month granularity as a result of the transformation applied in the `loadBirthData` function.
...
Besides the apparent seasonality to the birth data, it looks like there is a downward trend and possibly a trough during 2010. Lets run a linear regression and overlay that.
...
The "Linear" line is a simple linear regression on the entire data set, while the "Linear < 2011" line shows a simple linear regression of only the data between 2007 - 2010. Likewise, the "Linear >= 2010" line shows a simple linear regression for the data between 2010 - 2012. As can be seen by the "Linear" line, there is definitely a negative trend overall, but not a drastic one... the slope is only `r sprintf("%f", fit$coefficients[2])`.  Notice that when we differentiate pre-2011 versus 2010 onward, the sign of the slope flips. This is evidence of the suspected trough in 2010 mentioned earlier. After 2010, there is a definite rise in births. 

Ok, lets bring unemployment data in the picture during this same time frame.
...
time of the great recession during 2008 and 2009. Let's overlay unemployment (slightly transformed through multiplication and rooting) on our Births chart and see how things line up.
...
Its hard to judge if there is a good relationship between births and unemployment, but it seems like there might be. After playing around with various functional forms, the following form provided a reasonable outcome. 
...
<center>
  <math>
    ...
    <msub><mi>B</mi><mn>1</mn></msub><mo>&InvisibleTimes;</mo><msub><mi>X</mi><mn>1</mn></msub><mo>+</mo>
    <msub><mi>B</mi><mn>2</mn></msub><mo>&InvisibleTimes;</mo><mroot><msub><mi>X</mi><mn>1</mn></msub><mn>3</mn></mroot><mo>+</mo>
    <msub><mi>B</mi><mn>3</mn></msub><mo>&InvisibleTimes;</mo><msub><mi>X</mi><mn>1</mn></msub><mo>&InvisibleTimes;</mo>
      <mroot><msub><mi>X</mi><mn>1</mn></msub><mn>3</mn></mroot>
  </math>
</center>
...
fit2 <- lm(B.Births ~ U.UnemploymentRate*nth.root(U.UnemploymentRate, modelRoot), data=tsData)
model.summary <- summary(fit2)
print(model.summary)
```

From the R<sup>2</sup> shown in the model summary shown above, the model explains approximately `r sprintf("%.1f", model.summary$r.squared * 100)` % of the total variability in the Birth data. Graphically, we can see that the model follows the linear regressions fairly closely, though the model doesn't predict any seasonality.
...
A couple of questions come to mind related to validation and improvement. How does this base model predict historical U.S. births given unemployment for the same period? Likewise, how can or should be integrate seasonality into the model? These questions and others are out of scope for this study but are natural extensions of this study and would add to the results presented herein.

### Conclusions

Though a validation using pre-2007 (or post-2012) Natality data was not performed, the model presented in this study provides a foundation for futher research into the relationship between unemployment rate and births in the United States. At a minimum, this study appears to confirm that the level of employment and by extension, economic activity, in the United States has an effect on birth in following months and years.
...
The `geoVisual` helper function was developed to integrate the shape data with the individual county birth rate data.
...
An animated version of the above visualization produced using the Gifmaker.me website can be found [on GitHub, in my DataAcqMgmt repository, here](https://raw.githubusercontent.com/dwdii/DataAcqMgmt/master/FinalProject/USBirthsGeographicAnimation.gif) `r citep(bibAniGif)`.
